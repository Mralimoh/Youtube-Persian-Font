...
(function() {
    ...
    const QUALITY_PATCHED = '__qualityPatched__';
    ...
    function generateCSS(font) {
        return `
            ...
            ytd-topbar-logo-renderer a#logo > div[hidden] {
            ...
            ytd-rich-item-renderer yt-lockup-metadata-view-model h3 a {
    ...
    function initPlayerWatcher() {
        ...
        const storageKey = Symbol('originalGetLevels');

        Object.defineProperty(win.HTMLDivElement.prototype, 'getAvailableQualityLevels', {
            configurable: true,
            enumerable: true,
            get() {
    ...
    function initLogoFix() {
        const fix = (e) => {
            const logoLink = e.target.closest('ytd-topbar-logo-renderer a#logo');
            if (logoLink) {
                if (logoLink.getAttribute('href') !== '/') {
                    logoLink.setAttribute('href', '/');
                }
                if (logoLink.getAttribute('title') !== 'YouTube Home') {
                    logoLink.setAttribute('title', 'YouTube Home');
                }
            }
        };
        document.addEventListener('pointerover', fix, true);
    }
